...
public class WorkerNodeDAO {
	...
	private final String insertWorkNodeSql = "insert into worker_node (id, host_name, port, type, launch_date, modified, created) values (?,?,?,?,?,?,?)";
			...
			try (PreparedStatement ps1 = connection
				...
				try (ResultSet resultSet = ps1.executeQuery()) {
					while (resultSet.next()) {
						...
						// 插入work node表数据
						try (PreparedStatement ps2 = connection.prepareStatement(insertWorkNodeSql)) {
							ps2.setLong(1, aLong);
						...
						// 修改tb_generator数据
						aLong++;
						try (PreparedStatement ps3 = connection.prepareStatement(updateGeneratorSql)) {
							ps3.setLong(1, aLong);
