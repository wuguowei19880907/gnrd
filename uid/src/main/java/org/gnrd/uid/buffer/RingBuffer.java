...
import org.gnrd.uid.utils.PaddedAtomicLong;
 ...
 * To prevent the side effect of False Sharing, {@link PaddedAtomicLong} is
 * using on 'tail' and 'cursor'
 * <p>
 * 
 * A ring buffer is consisted of:
 * <li><b>slots:</b> each element of the array is a slot, which is be set with a
 * UID
 * <li><b>flags:</b> flag array corresponding the same index with the slots,
 * indicates whether can take or put slot
...
public class RingBuffer {
	 ...
	 *            must be positive & a power of 2
	 * @param paddingFactor
	 *            percent in (0 - 100). When the count of rest available UIDs reach
	 *            the threshold, it will trigger padding buffer<br>
	 *            Sample: paddingFactor=20, bufferSize=1000 -> threshold=1000 * 20
	 *            /100, padding buffer will be triggered when tail-cursor<threshold
	 */
	 ...
	 * We use 'synchronized' to guarantee the UID fill in slot & publish new tail
	 * sequence as atomic operations<br>
	 * 
	 * <b>Note that: </b> It is recommended to put UID in a serialize way, cause we
	 * once batch generate a series UIDs and put the one by one into the buffer, so
	 * it is unnecessary put in multi-threads
	 *
	 * @param uid
	 * @return false means that the buffer is full, apply
	 *         {@link RejectedPutBufferHandler}
	 */
	public synchronized boolean put(long uid) {
		long currentTail = tail.get();
		long currentCursor = cursor.get();

		// tail catches the cursor, means that you can't put any cause of RingBuffer is
		// full
		...
		// The atomicity of operations above, guarantees by 'synchronized'. In another
		// word,
		// the take operation can't consume the UID we just put, until the tail is
		// published(tail.incrementAndGet())
		return true;
	}

	/**
	 * Take an UID of the ring at the next cursor, this is a lock free operation by
	 * using atomic cursor
	 * <p>
	 * 
	 ...
	 * {@link RejectedTakeBufferHandler} will be applied<br>
	 * 
	 * @return UID
	...
	public long take() {
		...
		if (currentTail - nextCursor < paddingThreshold) {
			LOGGER.debug("Reach the padding threshold:{}. tail:{}, cursor:{}, rest:{}", paddingThreshold, currentTail,
		...
		// Note that: Step 2,3 can not swap. If we set flag before get value of slot,
		// the producer may overwrite the
		// slot with a new UID, and this may cause the consumer take the UID twice after
		// walk a round the ring
		return uid;
	}
	...
	protected void discardPutBuffer(RingBuffer ringBuffer, long uid) {
		LOGGER.debug("Rejected putting buffer for uid:{}. {}", uid, ringBuffer);
	}

	/**
	 * Policy for {@link RejectedTakeBufferHandler}, throws {@link RuntimeException}
	 * after logging
	 */
	protected void exceptionRejectedTakeBuffer(RingBuffer ringBuffer) {
		LOGGER.debug("Rejected take buffer. {}", ringBuffer);
	...
	public String toString() {
		...
		builder.append("RingBuffer [bufferSize=").append(bufferSize).append(", tail=").append(tail).append(", cursor=")
				.append(cursor).append(", paddingThreshold=").append(paddingThreshold).append("]");
